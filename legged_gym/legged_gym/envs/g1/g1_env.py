...
class G1Robot(LeggedRobot):
    ...
    def compute_observations(self):
        ...
        # Privileged obs now mirror base env: obs + base_lin_vel + contact forces + terrain heights
    ...
    def _reward_feet_too_near(self):
        # feet_pos = self._rigid_body_state_reshaped[:, self.feet_indices, :3]
        distance = torch.norm(self.feet_pos[:, 0, :] - self.feet_pos[:, 1, :], dim=-1)
        return (0.2 - distance).clamp(min=0)